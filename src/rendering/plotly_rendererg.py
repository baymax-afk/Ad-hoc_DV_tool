...
def _scatter_geo(df: pd.DataFrame, spec: ChartSpec) -> go.Figure:
    ...
    return px.scatter_geo(
        df, lat=spec.x_col, lon=spec.y_col,
        size=spec.size_col,
        color=spec.color_col,
        hover_name=spec.color_col,
...
def _state_choropleth(df: pd.DataFrame, spec: ChartSpec) -> go.Figure:
    ...
    fig = go.Figure(data=go.Choropleth(
        locations=df_copy[spec.x_col],
        z=df_copy[spec.y_col],
        locationmode="USA-states",
        colorscale="Viridis",
        text=df_copy.get(spec.color_col, df_copy[spec.x_col]),
        colorbar_title=spec.y_label or spec.y_col,
    ))
